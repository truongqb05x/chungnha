...
const CONFIG = {
  ...
  async function fetchMenus(view = 'all') {
    ...
    try {
      const response = await fetch(`${CONFIG.apiEndpoints.menus}?${params}`);
  ...
  async function renderCalendar() {
    ...
    try {
      ...
      const params = new URLSearchParams({ start_date: startDate, end_date: endDate, view: 'calendar' });
      const response = await fetch(`${CONFIG.apiEndpoints.menus}?${params}`);
      const { menus, error } = await response.json();
  ...
  async function handleEdit(id) {
    try {
      const response = await fetch(`${CONFIG.apiEndpoints.menu}/${id}`);
  ...
  function handleDelete(id) {
    showConfirm(
      ...
      async () => {
        try {
          const response = await fetch(`${CONFIG.apiEndpoints.menu}/${id}`, { method: 'DELETE' });
          const result = await response.json();
          if (result.error) throw new Error(result.error);
